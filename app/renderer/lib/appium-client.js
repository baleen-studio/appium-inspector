...
const {TAP, SWIPE, GESTURE, TEXT} = SCREENSHOT_INTERACTION_MODE;
...
export default class AppiumClient {
  ...
  async run(params) {
    ...
    if (methodName) {
      if (elementId) {
        ...
        res = await this.executeMethod({methodName, args, skipRefresh, skipScreenshot, appMode, value});
      }
  ...
  async executeMethod({elementId, methodName, args, skipRefresh, skipScreenshot, appMode}) {
    ...
    if (elementId) {
      ...
      if (!cachedEl) {
        cachedEl = this.fetchElement("id", elementId);
      }

      if (Object.keys(cachedEl).indexOf('variableName') !== -1 || !cachedEl.variableName) {
      ...
      if ([TAP, SWIPE, GESTURE, TEXT].includes(methodName)) {
